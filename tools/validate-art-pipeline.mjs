...
const contract=readJson(contractPath),profiles=readJson(`${dir}/planet-profiles.json`),stations=readJson(`${dir}/station-activity.json`),atlas=readJson(`${dir}/planet-art-atlas.json`),budget=readJson(`${dir}/psp-art-budget.json`);
...
let profileCount=0;const planetKeys=new Set();for(const s of profiles.systems||[])for(const p of s.planets||[]){profileCount++;const key=`${s.system}/${p.body}`;check(!planetKeys.has(key),`duplicate planet identity ${key}`);planetKeys.add(key);check(p.type==='ocean'?p.family===0:p.family===1+(p.seed%4),`profile family mismatch ${key}`);}
...
const stationKeys=new Set();for(const r of stations.records||[]){const key=`${r.system}/${r.hub}`;check(!stationKeys.has(key),`duplicate station identity ${key}`);stationKeys.add(key);}check(stations.records?.every(r=>r.architecture>=0&&r.architecture<6&&r.arrangement>=0&&r.arrangement<8&&r.landmark>=0&&r.landmark<8&&r.windows>=4&&r.windows<=13&&r.traffic>=2&&r.traffic<=10&&r.security>=1&&r.security<=7&&r.service>=1&&r.service<=5),'station profile/activity bounds invalid');
check(atlas.atlas_version===1&&atlas.variants===160&&atlas.tile?.[0]===32&&atlas.tile?.[1]===32,'planet art atlas metadata invalid');
check(budget.budget_report_version===1&&budget.contract_version===contract.contract_version,'PSP art budget report missing or version mismatch');
check(budget.projections?.room_candidate_packed4bit_bytes===28560&&budget.projections?.room_candidate_palette_bytes===44,'room packed4bit budget projection invalid');
check(budget.runtime_guardrails?.runtime_allocations===0&&budget.runtime_guardrails?.collision_from_art===false,'PSP runtime guardrails missing from budget report');
check(Array.isArray(budget.selector?.fingerprint_inputs)&&budget.selector.runtime_writes?.length===0,'selector fingerprint contract is not read-only');
check(Object.prototype.hasOwnProperty.call(budget.runtime_guardrails,'plan_bytes')&&Object.prototype.hasOwnProperty.call(budget.runtime_guardrails,'peak_temp_memory_bytes')&&Object.prototype.hasOwnProperty.call(budget.runtime_guardrails,'draw_calls_per_frame'),'PSP plan/temp/draw budget fields missing');
