...
const ids=new Set(),hubIds=new Set(),roomCounts=new Map();let pending=0,fallback=0,authored=0;
...
const report={report_version:1,register_schema:data.schema_version,register_source_sha:data.source_sha,register_file_sha256:sourceHash,coverage:{systems:256,hubs:hubIds.size,rooms:ids.size,rooms_per_hub:7,selector_pending:pending,generic_fallback:fallback,authored_exception_mentions:authored},activation:{runtime_art_activation:false,capability_manifest_read_only:true,save_schema_change:false,selector_version:'pending',selector_hash:'pending',generator_version:'pending',generator_hash:'pending',native_capture:'pending',draw_cost:'pending'},notes:['This is a build-time planning validation report.','Do not treat planned procedural variants as shipped art or gameplay capability.','Generic station behavior remains the truthful fallback until freeze gates pass.']};
