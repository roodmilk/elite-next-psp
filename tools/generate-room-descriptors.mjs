...
for(const r of source.records||[]){
  ...
  records.push({galaxy_id:0,system_id:r.system_id,hub_index:r.hub_index,room_id,room_code:r.room_code,family_id,arrangement_id:layout_id,landmark_id,material_id,art_version:1,exception_id,hub_role:r.hub_role,selector_version:1,selector_key:key,selector_fingerprint:fingerprint,family:r.family_proposal,layout:r.layout_variant,landmark:r.hero_landmark,visual_target:r.visual_plan?.target||'procedural_variant',fallback:r.visual_plan?.current_fallback||'generic renderer',fidelity_status:'pending',current_actions:[...(r.current_actions||[])],exits:[...(r.exits||[])],capability_source:r.capability_manifest?.source||'authoritative Station/Gameplay record',art_writes:[],runtime_activation:false});
}
