...
**Working tip (Commander: BIG RELEASES ONLY):** Story Act III–IV page scripts + dialogue chrome (COMMANDER bubble, no `YOU:`/`YOU SPOKE`; ask→answer realigned; codas through berth / `SAGA_ACT4_END`) plus ART DIRECTOR space animation kit (`space-animation-kit.h`, station window beacons) folded onto this branch. Do not micro-tag — next GitHub Release only when a substantial story+fun combined drop is ready.
