...
**Story pending on tip (untagged):** Act III–IV page scripts + dialogue chrome fix (COMMANDER bubble, no `YOU:`/`YOU SPOKE`; ask→answer realigned; codas through berth / `SAGA_ACT4_END`). **BIG RELEASES ONLY** — Story Manager does not micro-tag; coordinate with Gameplay Designer (`bc-dde672be-…`) for the next substantial story+fun drop.
