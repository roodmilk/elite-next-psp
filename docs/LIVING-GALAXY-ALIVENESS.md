...
**Status:** Phase 1 shipping on tip (untagged until next big drop)  
...
1. **Abstract registry** — `TravellerLive[12]`: current system, destination, met flag. Lives on `Game` (session). Save V12 later.
...
- Optional: one freighter callsign that mirrors a traveller capital route
