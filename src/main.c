...
#define TEST_INIT() do{game_init(&game);deck_reset();story_complete(&game);paused=0;selected_target=0;autoaim=0;scan_cat=2;tracked_mission=0;}while(0)
 ...
 input(PSP_CTRL_LEFT,PSP_CTRL_SQUARE|PSP_CTRL_LEFT,.016f,0,0);INPUT_CHECK(page==FLIGHT&&IS_NPC_ID(selected_target)&&!npc_is_hostile(&game.npc[selected_target-BODY_COUNT-1]),"Square+Left tabs to non-hostile ships without opening the computer");input(PSP_CTRL_RIGHT,PSP_CTRL_SQUARE|PSP_CTRL_RIGHT,.016f,0,0);INPUT_CHECK(page==FLIGHT&&selected_target==0,"Square+Right tabs back to the station");input(PSP_CTRL_RIGHT,PSP_CTRL_SQUARE|PSP_CTRL_RIGHT,.016f,0,0);INPUT_CHECK(page==FLIGHT&&(IS_DEBRIS_ID(selected_target)||IS_ANOMALY_ID(selected_target)),"Square+Right tabs to other contacts");input(PSP_CTRL_RIGHT,PSP_CTRL_SQUARE|PSP_CTRL_RIGHT,.016f,0,0);INPUT_CHECK(page==FLIGHT&&IS_NPC_ID(selected_target)&&npc_is_hostile(&game.npc[selected_target-BODY_COUNT-1])&&scan_cat==4,"Square+Right tabs to the ENEMIES band");input(PSP_CTRL_RIGHT,PSP_CTRL_SQUARE|PSP_CTRL_RIGHT,.016f,0,0);INPUT_CHECK(page==FLIGHT&&selected_target>=2&&selected_target<=BODY_COUNT,"Square+Right tabs to planets");int planet_lock=selected_target;input(PSP_CTRL_UP,PSP_CTRL_SQUARE|PSP_CTRL_UP,.016f,0,0);INPUT_CHECK(page==FLIGHT&&selected_target>=2&&selected_target<=BODY_COUNT&&selected_target!=planet_lock,"Square+Up cycles the highlighted planet band");
