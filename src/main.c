...
static void input(unsigned pressed,unsigned held,float dt,float ax,float ay){
 ...
 if(tracked_mission==0&&prologue_brief_locked()){
  ...
  else if(page==INVENTORY&&game.docked&&(pressed&PSP_CTRL_CROSS)){if(row>=0&&row<FIT_SLOTS)unequip_slot(row,1);}
...
#define TEST_INIT() do{game_init(&game);deck_reset();story_complete(&game);paused=0;selected_target=0;autoaim=0;scan_cat=2;tracked_mission=0;prologue_brief_beat=0;prologue_brief_echo=0;saga_brief_beat=0;saga_brief_echo=0;saga_brief_chapter=-1;saga_coda_pending=-1;}while(0)
 ...
 game_init(&game);deck_reset();change_page(HOME);row=20;input(PSP_CTRL_CROSS,0,.016f,0,0);INPUT_CHECK(page==HOME&&game.story==STORY_BRIEF&&strstr(game.message,"Finish the optional flight guide")!=0,"station tour waits for STORY_FREE while docked at Lave");
 TEST_INIT();game.system=7;game.docked=1;change_page(HOME);row=20;input(PSP_CTRL_CROSS,0,.016f,0,0);INPUT_CHECK(page==WALK&&walk_kind==0&&game.system==7,"STORY_FREE commander docked at Lave begins the station tour");
