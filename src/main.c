...
static void change_page(int p){
 ...
 if(p==CHART){update_nearby();chart_cursor=tracked_mission==0&&game.campaign_stage>=6&&game.saga_step&&game.saga_chapter<SAGA_COUNT?game.saga_dest:game.destination;for(int i=0;i<near_count;i++)if(nearby[i]==game.destination)row=i;}
...
static void input(unsigned pressed,unsigned held,float dt,float ax,float ay){
 ...
 else if(page==FLIGHT&&game.jump<=0&&!game.dead&&!game.dock_stage&&(pressed&PSP_CTRL_SQUARE)&&(held&PSP_CTRL_RTRIGGER)){int hostile=nearest_hostile_target();sq_arm=0;if(hostile>=0){selected_target=hostile;scan_cat=1;autoaim=1;story_event(&game,STORY_EV_TARGET);if(selected_target==0)campaign_event(&game,CP_LOCK);message(&game,"Nearest hostile locked.");game.cue=SFX_UI;}else message(&game,"No hostile contacts.");}
 else if(page==FLIGHT&&game.jump<=0&&!game.dead&&!game.dock_stage&&(pressed&PSP_CTRL_SQUARE))sq_arm=1;
 if(page==FLIGHT&&game.jump<=0&&!game.dead&&!game.dock_stage&&(released&PSP_CTRL_SQUARE)&&sq_arm){sq_arm=0;if(valid_target(selected_target))scan_cat=target_category(selected_target);change_page(TARGETING);target_count=collect_scan_ids(target_ids,scan_cat);if(row>=target_count)row=0;story_event(&game,STORY_EV_TARGET);if(selected_target==0)campaign_event(&game,CP_LOCK);}
 ...
 if(page==HELP&&(pressed&(PSP_CTRL_LEFT|PSP_CTRL_RIGHT)))help_tab=(help_tab+(pressed&PSP_CTRL_RIGHT?1:3))%4;
   if(page==TARGETING&&(pressed&(PSP_CTRL_LTRIGGER|PSP_CTRL_RTRIGGER))){step_scan_cat(pressed&PSP_CTRL_RTRIGGER?1:-1);row=0;}
   ...
   if(page==CODEX&&(pressed&(PSP_CTRL_LTRIGGER|PSP_CTRL_RTRIGGER))){codex_tab=(codex_tab+(pressed&PSP_CTRL_RTRIGGER?1:4))%5;row=0;}
 ...
 if(tracked_mission==0&&game.campaign_stage==0){if(row==0)narrative_do(CAMPAIGN);else if(row<3)game.campaign_choice=row;else change_page(HELP);}
 else if(tracked_mission==0&&game.campaign_stage>=6){if(game.saga_chapter<SAGA_COUNT&&!game.saga_step)saga_begin(&game);else if(game.saga_chapter<SAGA_COUNT&&saga_beats[game.saga_chapter].kind==SAGA_CHOICE){game.saga_choice=row+1;saga_advance(&game);row=0;}else if(!saga_advance(&game)){int hops=0,hop=saga_next_hop(&game,&hops);if(hop<0){message(&game,"No route with this drive. Fit more jump range.");}else{game.destination=hop;change_page(CHART);char note[96];snprintf(note,sizeof(note),hop==game.saga_dest?"Destination selected: %s.":"Next jump: %s. Final destination: %s.",game.systems[hop].name,game.systems[game.saga_dest].name);message(&game,note);}}}
 ...
 else if(page==GALNET&&(galnet_tab==3||galnet_tab==5)){if(galnet_tab==3&&(pressed&PSP_CTRL_CROSS))spacebook_likes[game.system]^=1u<<row;if(pressed&PSP_CTRL_TRIANGLE)spacebook_comments=!spacebook_comments;}
 else if(page==RADIO){if(pressed&PSP_CTRL_TRIANGLE){radio_volume=radio_volume?0:5;radio_dirty=1;}else if(row<RADIO_STATION_COUNT&&(pressed&PSP_CTRL_CROSS)){radio_tune(row);}else if(row>=RADIO_STATION_COUNT&&(pressed&(PSP_CTRL_LEFT|PSP_CTRL_RIGHT))){radio_adjust(row==RADIO_STATION_COUNT+1,pressed&PSP_CTRL_RIGHT?1:-1);}}else if(page==DEBUG&&(pressed&PSP_CTRL_CROSS)){debug_action();}
  else if(page==MARKET&&game.docked){if(pressed&PSP_CTRL_RIGHT)trade(&game,row,1);if(pressed&PSP_CTRL_LEFT)trade(&game,row,0);}
  else if(page==CHART){
   if(pressed&PSP_CTRL_TRIANGLE){chart_mode=!chart_mode;if(chart_mode)chart_cursor=tracked_mission==0&&game.campaign_stage>=6&&game.saga_step&&game.saga_chapter<SAGA_COUNT?game.saga_dest:game.destination;message(&game,chart_mode?"Galaxy overview: all 256 systems.":"Nearby jump list.");}
   else if(chart_mode&&(pressed&PSP_CTRL_CROSS)){Game route=game;route.fuel=(float)player_ships[game.ship].range;int jumps=0,hop=route_next_hop(&route,chart_cursor,&jumps);if(hop<0)message(&game,"No route with the fitted jump drive.");else{game.destination=hop;chart_mode=0;update_nearby();row=0;for(int i=0;i<near_count;i++)if(nearby[i]==hop)row=i;char note[96];snprintf(note,sizeof(note),"Route to %s: %d jump%s. Next: %s.",game.systems[chart_cursor].name,jumps,jumps==1?"":"s",game.systems[hop].name);message(&game,note);}}
...
#define TEST_INIT() do{game_init(&game);deck_reset();story_complete(&game);paused=0;selected_target=0;autoaim=0;scan_cat=2;tracked_mission=0;}while(0)
 ...
 TEST_INIT();launch(&game);page=FLIGHT;input(PSP_CTRL_SQUARE,PSP_CTRL_SQUARE,.016f,0,0);INPUT_CHECK(page==FLIGHT,"Square press waits for release before opening the computer");input(0,0,.016f,0,0);INPUT_CHECK(page==TARGETING&&target_count>0,"Square tap opens populated targeting computer");target_filter=2;target_refresh();INPUT_CHECK(target_count>0,"hostile targeting filter finds pirate traffic");target_filter=10;target_refresh();INPUT_CHECK(target_count>=1,"anomaly filter lists rare system echoes");galnet_tab=3;INPUT_CHECK(galnet_rows()==7,"SpaceBook provides a scrollable generated feed");
 change_page(HOME);row=15;input(PSP_CTRL_CROSS,0,.016f,0,0);INPUT_CHECK(page==CODEX,"command deck opens the Discovery Codex");
 TEST_INIT();launch(&game);page=FLIGHT;input(PSP_CTRL_LEFT,PSP_CTRL_SQUARE|PSP_CTRL_LEFT,.016f,0,0);INPUT_CHECK(page==FLIGHT&&IS_NPC_ID(selected_target),"Square+Left tabs to ships without opening the computer");input(PSP_CTRL_RIGHT,PSP_CTRL_SQUARE|PSP_CTRL_RIGHT,.016f,0,0);INPUT_CHECK(page==FLIGHT&&selected_target==0,"Square+Right tabs back to the station");input(PSP_CTRL_RIGHT,PSP_CTRL_SQUARE|PSP_CTRL_RIGHT,.016f,0,0);INPUT_CHECK(page==FLIGHT&&(IS_DEBRIS_ID(selected_target)||IS_ANOMALY_ID(selected_target)),"Square+Right tabs to other contacts");input(PSP_CTRL_RIGHT,PSP_CTRL_SQUARE|PSP_CTRL_RIGHT,.016f,0,0);INPUT_CHECK(page==FLIGHT&&selected_target>=2&&selected_target<=BODY_COUNT,"Square+Right tabs to planets");int planet_lock=selected_target;input(PSP_CTRL_UP,PSP_CTRL_SQUARE|PSP_CTRL_UP,.016f,0,0);INPUT_CHECK(page==FLIGHT&&selected_target>=2&&selected_target<=BODY_COUNT&&selected_target!=planet_lock,"Square+Up cycles the highlighted planet band");
 ...
 TEST_INIT();game.system=0;launch(&game);page=FLIGHT;input(PSP_CTRL_SQUARE,PSP_CTRL_SQUARE|PSP_CTRL_RTRIGGER,.016f,0,0);INPUT_CHECK(page==FLIGHT&&selected_target>BODY_COUNT&&game.npc[selected_target-BODY_COUNT-1].role==PIRATES,"R and Square locks nearest hostile without opening menu");int shots=game.shots;input(PSP_CTRL_CROSS,PSP_CTRL_CROSS|PSP_CTRL_LTRIGGER,.016f,0,0);INPUT_CHECK(game.missile_time>0&&game.missiles==0&&game.shots==shots,"L and X launches missile without firing laser");
...
int main(void){
 ...
 FILE *factionflag=fopen("open-factions.flag","r");if(factionflag){fclose(factionflag);change_page(FACTIONS);row=1;}FILE *comfortflag=fopen("open-comfort.flag","r");if(comfortflag){fclose(comfortflag);change_page(COMFORT);}FILE *homeflag=fopen("open-home.flag","r");if(homeflag){int item=0;fscanf(homeflag,"%d",&item);fclose(homeflag);change_page(HOME);row=item>=0&&item<DECK_ITEMS?item:0;game.voice_time=0;}FILE *cpflag=fopen("open-campaign.flag","r");if(cpflag){int stage=0,flying=0;fscanf(cpflag,"%d %d",&stage,&flying);fclose(cpflag);if(flying)launch(&game);game.campaign_stage=stage>=0&&stage<=6?stage:0;game.voice_time=0;tracked_mission=0;change_page(CAMPAIGN);}FILE *guildflag=fopen("open-guild.flag","r");if(guildflag){int chapter=0,ready=0;fscanf(guildflag,"%d %d",&chapter,&ready);fclose(guildflag);game.guild_chapter=chapter>=0&&chapter<=4?chapter:0;game.guild_flags=ready?31:0;game.voice_time=0;tracked_mission=1;change_page(CAMPAIGN);}
