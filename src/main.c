...
#define TEST_INIT() do{game_init(&game);deck_reset();story_complete(&game);paused=0;selected_target=0;autoaim=0;scan_cat=2;tracked_mission=0;}while(0)
 ...
 TEST_INIT();game.system=0;launch(&game);page=FLIGHT;input(PSP_CTRL_SQUARE,PSP_CTRL_SQUARE|PSP_CTRL_RTRIGGER,.016f,0,0);INPUT_CHECK(page==FLIGHT&&selected_target>BODY_COUNT&&game.npc[selected_target-BODY_COUNT-1].role==PIRATES&&scan_cat==4,"R and Square locks nearest hostile on the ENEMIES band");int shots=game.shots;input(PSP_CTRL_CROSS,PSP_CTRL_CROSS|PSP_CTRL_LTRIGGER,.016f,0,0);INPUT_CHECK(game.missile_time>0&&game.missiles==0&&game.shots==shots,"L and X launches missile without firing laser");
