...
static int mp3_source_frame(int station,int *left,int *right){
 if(!radio_track_count[station])return 0;
...
static int audio_worker(SceSize n,void *a){
 ...
 while(audio_run){
  ...
  for(int i=0;i<AUDIO_FRAMES;i++){
   ...
   int wanted=radio_station!=station?0:radio_volume*100;
   ...
   if(!mp3_sample(station,&music_l,&music_r))radio_synth_sample(&synth,&music_l,&music_r);
...
static void audio_stop(void){
 audio_run=0;
 if(audio_thread>=0){SceUInt timeout=500000;if(sceKernelWaitThreadEnd(audio_thread,&timeout)<0)sceKernelTerminateThread(audio_thread);sceKernelDeleteThread(audio_thread);audio_thread=-1;}
