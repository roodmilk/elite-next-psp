...
int land_planet(Game *g){
 ...
 int bit=1<<(g->planet-1),first_landing=!(g->landed_planets[g->system]&bit);g->landed_planets[g->system]|=(uint8_t)bit;
 message(g,"Landed. O to walk. Triangle to take off.");if(first_landing){char brief[160];snprintf(brief,sizeof(brief),"Planet %s added to your Discovery Codex. Pad locked. Circle to walk.",g->bodies[g->planet].name);speak(g,VOICE_COMP,brief);}else speak(g,VOICE_COMP,"Pad locked. Circle to walk.");return 1;
}
