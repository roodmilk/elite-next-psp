...
int police_fine(const Game *g){return wanted_level(g)*500;}
int police_resolve(Game *g,int jail){if(!g->police_stop)return 0;int cost=jail?police_fine(g)/2:police_fine(g);if(!jail&&g->credits<cost){message(g,"Not enough units. Press O for custody.");return 0;}if(cost>g->credits)cost=g->credits;g->credits-=cost;g->legal=0;g->wanted[g->system]=0;g->police_stop=0;g->attacked=0;g->speed=0;g->boost=0;g->cue=SFX_UI;if(jail){g->docked=1;g->pos=(Vec3){0,0,3200};g->energy=100;message(g,"Station custody complete. Bribe paid.");}else message(g,"Fine paid. Local warrant cleared.");return 1;}
int police_escape(Game *g){if(!g->police_stop)return 0;g->police_stop=0;add_crime(g,8);g->police_grace=7;g->attacked=7;g->speed=fmaxf(g->speed,player_ships[g->ship].speed*.8f);g->boost=0;for(int i=0;i<NPC_COUNT;i++)if(g->npc[i].alive&&g->npc[i].role==LAW)g->npc[i].target=-2;g->cue=SFX_ALERT;message(g,"You ran. Local warrant raised; patrols are pursuing.");speak(g,VOICE_LAW,"Suspect fleeing. All patrols intercept.");return 1;}
...
void game_init(Game *g){memset(g,0,sizeof(*g));g->rng=0x19841991;g->ai_phase=-1;galaxy(g->systems);g->system=7;g->destination=129;g->route_goal=-1;g->credits=1000;g->fuel=60;g->energy=100;g->docked=1;g->contract=-1;g->mission_target=-1;g->missile_target=-1;g->incoming_source=-1;g->approach=-1;g->planet=-1;g->missiles=1;g->pip_sys=2;g->pip_eng=2;g->pip_wep=4;market(g);game_spawn(g);g->cargo[0]=2;message(g,"X opens the deck.");speak(g,VOICE_KEI,"Kei. Ryn's missing. Welcome aboard.");}
...
int trade(Game *g,int i,int buy){if(!g->docked||i<0||i>=GOODS)return 0;
 if(buy){if(g->credits<g->price[i]||g->stock[i]<1||(goods[i].unit=='t'&&cargo_used(g)>=cargo_capacity(g))){message(g,"Check units, stock and cargo space.");return 0;}g->credits-=g->price[i];g->stock[i]--;g->cargo[i]++;if(i==3||i==6||i==10){add_crime(g,1);message(g,"Restricted goods purchased. Local warrant increased.");}}
 ...
 if(!(buy&&(i==3||i==6||i==10))){char note[64];snprintf(note,sizeof(note),buy?"Bought %s.":"Sold %s.",goods[i].name);message(g,note);}
 return 1;
}
int buy_ship(Game *g,int i){if(!g->docked||i<0||i>=player_ship_count)return 0;if(i==g->ship){message(g,"Already on this ship.");return 0;}
 ...
 if(g->credits<cost||cargo_used(g)>player_ships[i].capacity+((g->upgrades&8)?8:0)){message(g,"Not enough units, or cargo will not fit.");return 0;}
...
int salvage(Game *g,int id){
 ...
 if(d->wreck){g->credits+=d->qty;snprintf(note,sizeof(note),"HULL SALVAGE RECOVERED: %.1f UNITS.",d->qty*.1f);}
  ...
  else {g->cargo[item]+=d->qty;if(item==3||item==6||item==10){add_crime(g,1);snprintf(note,sizeof(note),"SALVAGED RESTRICTED %s. LOCAL WARRANT INCREASED.",goods[item].name);}else snprintf(note,sizeof(note),"SALVAGED %d%c %s.",d->qty,goods[item].unit,goods[item].name);}
 }
...
int survey_scan(Game *g){
 ...
 int best=-1;float range=380;for(int i=0;i<LIFE_COUNT;i++)if(g->life[i].alive&&!g->life[i].scanned){float d=length(sub(g->life[i].pos,g->pos));if(d<range){range=d;best=i;}}
 ...
 else {g->scanned_minerals++;if(cargo_used(g)>=cargo_capacity(g)){g->credits+=40;message(g,"Mineral logged. Hold full; ore sold for 4.0 units.");}else {g->cargo[12]++;message(g,"Mineral deposit scanned. 1t minerals collected.");}}
...
int mine_rock(Game *g,int id){
 ...
 d->health-=(g->laser?36:18)*(0.50f+0.25f*g->pip_wep);d->flash=.22f;g->cue=SFX_MINE;
 if(d->health<=0){
...
void game_tick(Game *g,float dt,float turn,float pitch,int throttle,int fire){
 ...
 g->time+=dt;g->message_time-=dt;if(g->message_time<0)g->message_time=0;g->attacked=fmaxf(0,g->attacked-dt);g->collision=fmaxf(0,g->collision-dt);if(g->incoming_missile>0){if(g->boost&&g->speed>player_ships[g->ship].speed*4){g->incoming_missile=0;g->incoming_source=-1;message(g,"Incoming missile evaded.");}else {g->incoming_missile-=dt;if(g->incoming_missile<=0){g->energy-=25;g->attacked=3;g->incoming_source=-1;g->cue=SFX_HIT;message(g,"Missile impact. Shields damaged.");}}}
 ...
 if(fire&&g->shot<=.001f&&g->heat<80){g->shot=.18f;g->heat+=12;g->shots++;g->cue=SFX_LASER;float closest=2200;int target=-1;
  ...
  else if(target>=0)hit(g,target,(int)((g->laser?36:18)*(0.50f+0.25f*g->pip_wep)),1);
 }
 freight_update(g,dt);
...
static int load_game_file(Game *g,const char *path){Save s;memset(&s,0,sizeof(s));FILE *f=fopen(path,"rb");if(!f)return 0;int warrants[256]={0},upgrades=0,mission[8]={MISSION_DELIVERY,0,-1,1,7,0,MISSION_DELIVERY,7}; int extra[5]={0,0,0,0,0};uint8_t visited[32]={0};int jobhead[2]={0,0};Job packed[MISSION_SLOTS];memset(packed,0,sizeof(packed));int storypack[8]={STORY_FREE,0xffff,2,2,4,0,0,0};uint32_t cp[5]={0},saga[10]={0},route_goal_u=0xffffffffu;float cp_distance=0,cp_fuel=0;int route_goal=-1;int ok=fread(&s,1,sizeof(s),f)==sizeof(s);if(ok&&s.version>=2)ok=fread(warrants,1,sizeof(warrants),f)==sizeof(warrants);if(ok&&s.version>=3)ok=fread(&upgrades,1,sizeof(upgrades),f)==sizeof(upgrades);if(ok&&s.version>=4)ok=fread(mission,1,sizeof(mission),f)==sizeof(mission);if(ok&&s.version>=5){ok=fread(extra,1,sizeof(extra),f)==sizeof(extra);if(ok)ok=fread(visited,1,sizeof(visited),f)==sizeof(visited);}if(ok&&s.version>=6){ok=fread(jobhead,1,sizeof(jobhead),f)==sizeof(jobhead);if(ok)ok=fread(packed,1,sizeof(packed),f)==sizeof(packed);}if(ok&&s.version>=7)ok=fread(storypack,1,sizeof(storypack),f)==sizeof(storypack);if(ok&&s.version>=8){for(int i=0;ok&&i<5;i++)ok=load_u32(f,&cp[i]);memcpy(&cp_distance,&cp[3],4);memcpy(&cp_fuel,&cp[4],4);if(cp[0]>6||cp[1]>2||cp[2]>7||!isfinite(cp_distance)||cp_distance<0||cp_distance>600||!isfinite(cp_fuel)||cp_fuel<0||cp_fuel>255)ok=0;}if(ok&&s.version>=9){for(int i=0;ok&&i<10;i++)ok=load_u32(f,&saga[i]);if(saga[0]>SAGA_COUNT||saga[1]>1||saga[3]>3||saga[4]>255)ok=0;}if(ok&&s.version>=10){ok=load_u32(f,&route_goal_u);if(ok){if(route_goal_u==0xffffffffu)route_goal=-1;else if(route_goal_u<=255)route_goal=(int)route_goal_u;else ok=0;}}if(ok)ok=save_verify_end(f,s.version);fclose(f);for(int i=0;i<256;i++)if(warrants[i]<0||warrants[i]>1000)ok=0;
 ...
 if(!g){return 1;}
 game_init(g);if(s.version>=9){g->saga_chapter=(int)saga[0];g->saga_step=(int)saga[1];g->saga_flags=(int)saga[2];g->saga_choice=(int)saga[3];g->saga_dest=(int)saga[4];g->saga_start=(int)saga[5];for(int i=0;i<4;i++)g->saga_trust[i]=(int)saga[6+i];}g->campaign_stage=(int)cp[0];g->campaign_choice=(int)cp[1];g->campaign_flags=(int)cp[2];g->campaign_distance=cp_distance;g->campaign_fuel=cp_fuel;g->system=s.system;g->destination=s.destination;g->route_goal=s.version>=10?route_goal:-1;g->credits=s.credits;g->kills=s.kills;g->legal=s.legal;memcpy(g->wanted,warrants,sizeof(warrants));g->wanted[g->system]=s.legal;g->ship=s.ship;g->laser=s.laser;g->missiles=s.missiles;g->fuel=s.fuel;g->contract=s.contract;g->contract_reward=s.reward;g->contract_time=s.remaining;g->upgrades=upgrades;g->mission_type=mission[0];g->mission_stage=mission[1];g->mission_target=mission[2];g->mission_item=mission[3];g->mission_origin=mission[4];g->mission_result=mission[5];g->last_mission_type=mission[6];g->last_mission_system=mission[7];
...
int game_tests(const char *path){FILE *f=fopen(path,"w");if(!f)return 1;int fails=0;Game g;game_init(&g);
 ...
 game_init(&g);add_crime(&g,15);CHECK(wanted_level(&g)==3&&g.wanted[g.system]==15,"crime raises the current system wanted level");g.credits=2000;g.police_stop=1;int fine=police_fine(&g);CHECK(police_resolve(&g,0)&&g.credits==2000-fine&&g.legal==0,"paying police deducts exact fine and clears local warrant");
 add_crime(&g,10);g.credits=20;g.police_stop=1;CHECK(!police_resolve(&g,0)&&g.police_stop,"unaffordable fine keeps police choice open");CHECK(police_resolve(&g,1)&&g.docked&&g.credits==0&&g.legal==0,"custody returns to station and takes affordable release bribe");
 game_init(&g);launch(&g);add_crime(&g,10);g.police_stop=1;int before_run=g.legal;CHECK(police_escape(&g)&&!g.police_stop&&g.legal>before_run&&g.police_grace>0&&g.attacked>0,"running from law resumes flight, escalates warrant and starts pursuit");
 ...
 g.npc[36].pos=add(g.pos,(Vec3){0,0,300});game_tick(&g,.016f,0,0,0,0);CHECK(g.police_stop,"approaching police opens interception");Vec3 stopped=g.npc[36].pos;float frozen=g.time;game_tick(&g,.016f,1,1,1,1);CHECK(g.time==frozen&&length(sub(stopped,g.npc[36].pos))==0,"police encounter freezes world simulation");
 g.police_stop=0;for(int i=0;i<NPC_COUNT;i++)g.npc[i].alive=0;int origin=g.system;g.destination=dest;g.fuel=100;g.jump=.001f;game_tick(&g,.016f,0,0,0,0);CHECK(g.legal==0&&g.wanted[origin]==10,"warp leaves warrant behind in origin system");g.destination=origin;g.jump=.001f;game_tick(&g,.016f,0,0,0,0);CHECK(g.legal==10,"returning to original system restores its warrant");
