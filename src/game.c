...
int game_tests(const char *path){FILE *f=fopen(path,"w");if(!f)return 1;int fails=0;Game g;game_init(&g);
 ...
 {StationProfile s0=station_profile_for(&g.systems[0],0,0),s0_again=station_profile_for(&g.systems[0],0,0),s1=station_profile_for(&g.systems[0],0,1);CHECK(station_profile_valid(&s0)&&s0.seed==s0_again.seed&&s0.architecture==s0_again.architecture&&s0.arrangement==s0_again.arrangement&&s0.landmark==s0_again.landmark&&s0.windows==s0_again.windows,"station profile is deterministic and bounded");CHECK(s1.hub==1&&s1.seed!=s0.seed,"station profile distinguishes secondary hubs");}
 {const PlanetPackRecord *packed=planet_pack_find(0,1);if(packed){PlanetProfile live=planet_profile_for_body(&g.bodies[1]);CHECK(packed->seed==live.seed&&packed->family==live.family&&packed->terrain_style==live.terrain_style&&packed->palette==live.palette&&packed->sea_level==live.sea_level&&packed->prop_density==live.prop_density&&packed->activity_density==live.activity_density,"planet sidecar matches live deterministic profile");}}
