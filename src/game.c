...
int game_tests(const char *path){FILE *f=fopen(path,"w");if(!f)return 1;int fails=0;Game g;game_init(&g);
 ...
 {const StationPackRecord *packed=station_pack_find(0,0);if(packed){StationProfile live=station_profile_for(&g.systems[0],0,0);CHECK(packed->seed==live.seed&&packed->architecture==live.architecture&&packed->arrangement==live.arrangement&&packed->landmark==live.landmark&&packed->windows==live.windows&&packed->traffic==live.traffic&&packed->security==live.security&&packed->service==live.service,"station sidecar matches live C-parity profile");}}
