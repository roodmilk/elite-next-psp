...
int game_tests(const char *path){FILE *f=fopen(path,"w");if(!f)return 1;int fails=0;Game g;game_init(&g);
 ...
 {if(!station_pack_loaded)station_pack_load("station.content");const StationPackRecord *packed=station_pack_find(0,0);int matches=!station_pack_loaded;if(packed){StationProfile live=station_profile_for(&g.systems[0],0,0);matches=packed->seed==live.seed&&packed->architecture==live.architecture&&packed->arrangement==live.arrangement&&packed->landmark==live.landmark&&packed->windows==live.windows&&packed->traffic==live.traffic&&packed->security==live.security&&packed->service==live.service;}CHECK(matches,"station sidecar matches live C-parity profile when present");}
 {if(!planet_pack_loaded)planet_pack_load("planet.content");const PlanetPackRecord *packed=planet_pack_find(0,1);int matches=!planet_pack_loaded;if(packed){PlanetProfile live=planet_profile_for_body(&g.bodies[1]);matches=packed->seed==live.seed&&packed->family==live.family&&packed->terrain_style==live.terrain_style&&packed->palette==live.palette&&packed->sea_level==live.sea_level&&packed->prop_density==live.prop_density&&packed->activity_density==live.activity_density;}CHECK(matches,"planet sidecar matches live deterministic profile when present");}
