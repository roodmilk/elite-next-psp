...
int mission_offer_active(const Game *g,int offer){int dest=mission_destination(g,offer),type=mission_type_for_offer(g,offer);if(dest<0||!mission_offer_valid(g,offer,0))return 0;for(int i=0;i<g->job_n;i++)if(g->jobs[i].dest==dest&&g->jobs[i].type==type)return 1;return 0;}
