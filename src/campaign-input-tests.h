{
 ...
 INPUT_CHECK(page==FLIGHT&&!game.docked&&game.shots==0,"campaign UI: visible Launch ship reply launches without firing");
 ...
 input(PSP_CTRL_CROSS,0,.016f,0,0);INPUT_CHECK(page==HOME,"assignments: completed tracked briefing offers a working return to the deck");
 ...
 INPUT_CHECK(page==HOME&&game.story==STORY_FREE,"guide: retired optional coaching redirects to the main menu");
 ...
 INPUT_CHECK(strstr(prologue_brief_reply(0),"first flight")!=0,"chat flow: first reply asks Kei to explain the flight");
 input(PSP_CTRL_CROSS,0,.016f,0,0);
 INPUT_CHECK(!prologue_brief_echo&&prologue_brief_beat==1&&strstr(prologue_brief_line1(1),"no time limit"),"chat flow: Kei explains the assignment after the selected question");
 ...
 INPUT_CHECK(strstr(prologue_brief_reply(3),"Walk me through")&&strstr(prologue_brief_reply(4),"understand the plan"),"script: prologue asks stay spoken, not menu chrome");
 ...
 INPUT_CHECK(strstr(saga_beats[20].talk7,"Yes.")&&strstr(saga_beats[20].talk7,"working crews"),"script: Open Channel confirm ask gets crews-first yes");
 ...
 INPUT_CHECK(strstr(prologue_brief_line1(0),"Ryn")&&strstr(prologue_brief_line1(0),"three"),"script: prologue hook still names Ryn and the three missed calls");
 {
  ...
  INPUT_CHECK(!!strstr(guild_line(&echo,0),"survey"),"script: Guild opening explains the survey flight assignment");
