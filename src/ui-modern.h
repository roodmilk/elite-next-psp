...
static void mission_log(void){
 ...
 text(2,5,CYAN,"MISSIONS");text(43,5,DIM,"* TRACKED");
 int total=mission_log_count();
 if(row<0)row=0;if(row>=total)row=total-1;
 for(int i=0;i<total;i++){
  int y=7+i*2;int track=mission_track_at(i);
  if(i==row)selected(y);
  unsigned ink=track==tracked_mission?GOLD:i==row?WHITE:DIM;
 ...
 const char *objective=mission_track_at(row)==TRACK_STATION_TOUR?station_tour_objective():row==0?(game.campaign_stage>=6&&game.saga_chapter<SAGA_COUNT?saga_beats[game.saga_chapter].objective:campaign_task(&game)):row==1?guild_objective(&game):mission_objective_at(&game,row-2);
