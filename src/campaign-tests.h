static void campaign_tests(FILE *f,int *failures){
 ...
 lawful.saga_flags|=SAGA_CASE_HELD;lawful.system=7;saga_dock_event(&lawful);
 CHECK(!(lawful.saga_flags&SAGA_TIMESTAMP_FOUND),"saga: sealed receiver without the port stamp cannot compare evidence");
 lawful.saga_flags=0;lawful.saga_flags|=SAGA_STAMP_FOUND;saga_dock_event(&lawful);
 CHECK(!(lawful.saga_flags&SAGA_TIMESTAMP_FOUND),"saga: port stamp without the sealed receiver cannot compare evidence");
 lawful.saga_flags|=SAGA_CASE_HELD;lawful.system=6;saga_dock_event(&lawful);
  ...
  * After V7: campaign(20)+saga(40)+route(4)+pax(16)+travellers(48)+fit(8)+landings(256)+CRC(4)=396. */
 if(n>396){bytes[4]=7;FILE *v7=fopen("test-cp-v7.sav","wb");if(v7){fwrite(bytes,1,n-396,v7);fclose(v7);}}
