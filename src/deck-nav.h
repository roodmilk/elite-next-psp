...
#define DECK_ITEMS 25
#define DECK_GROUPS 5
static const int deck_rows[DECK_GROUPS][6]={{0,8,11,2,10,20},{1,3,4,21,23,24},{17,12,13,7,-1,-1},{15,14,22,-1,-1,-1},{5,6,16,19,9,-1}};
static const int deck_sizes[]={6,6,4,3,5};
...
static int deck_service_visible(int id){if(id==18)return 0;if(!tutorial_service(&game,id))return 0;if(id==3||id==4||id==12||id==23||id==24)return game.docked;return 1;}
...
static void deck_tab(int direction){int g=deck_group(row);deck_focus[g]=row;for(int k=0;k<DECK_GROUPS;k++){g=(g+DECK_GROUPS+direction)%DECK_GROUPS;int vis[6];if(deck_fill(g,vis)>0)break;}row=deck_focus[g];deck_clamp_row();deck_focus[g]=row;}
